...
| `replication_bandwidth` | `purefa_pod_replica_links_performance_bandwidth_bytes{dimension="bytes_per_sec_total"}` (→ MB/s) | Back-end | Aggregate replication-link throughput | Confirmed directly from the exporter's own collector source (`pod_replica_links_performance_collector.go`) — this metric doesn't appear in Pure's published `purefa-metrics.md` spec doc at all, which marks its pod-metrics section "TODO." Correlates with `replication_lag`: bandwidth pegged near the link's known capacity while lag climbs points at the link; flat bandwidth with rising lag points at local write-path contention instead — the rules engine now derives exactly this distinction automatically (a second cross-panel correlation finding, alongside the front-end/back-end one) |
...
| `node_cpu_busy` | `node_cpu_busy` (+ `node_cpu_busy_by_node`) | Back-end | Controller/node CPU busy percentage | Computed as a ratio of two counters' deltas between polls (NetApp's own documented single-sample formula is confirmed wrong — see the package doc comment) — this is the metric Pure's public endpoint doesn't have an equivalent for. The cluster-wide average has a real blind spot: two nodes at 95%/5% average to ~50%, comfortably under the 70% watch line, hiding a genuine single-node hot spot. `node_cpu_busy_by_node` (per-node delta, same formula) closes it — the dashboard's breakdown surfaces the worst node even when the average reads clean, and a masked node now also produces its own finding (and webhook) that the average alone wouldn't |
| `aggr_disk_busy` | `aggr_disk_busy` (+ `aggr_disk_busy_by_node`) | Back-end | Average disk busy percentage across all disks | Computed the same schema-driven way as `nic_utilization`: ONTAP's counter-tables API self-describes each counter's type and which counter it must be divided against, so this reads that schema live rather than assuming a fixed denominator. Same masking risk and fix as `node_cpu_busy` above, at the disk level — the breakdown is capped to the worst 10 disks (`topk(10, ...)` in the thresholds query), since a real cluster can have hundreds and the breakdown UI has no pagination of its own |
...
| `snapmirror_bandwidth` | `snapmirror_bandwidth_bytes` (rate, → MB/s), sourced from `total_transfer_bytes` | Back-end | Aggregate SnapMirror transfer rate | **Lower confidence than the rest of this file.** NetApp Harvest's own SnapMirror config (`conf/rest/9.12.0/snapmirror.yaml`) sources this field from `api/private/cli/snapmirror` — an unsupported CLI-passthrough endpoint, not the documented `api/snapmirror/relationships` resource `snapmirror_lag` uses. NetApp can change or remove `api/private/cli/*` without notice; if it's unavailable on your cluster (older ONTAP, restricted RBAC), this panel alone degrades to "unavailable" — `snapmirror_lag` and everything else here is unaffected |
