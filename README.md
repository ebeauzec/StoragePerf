...
- **Two report types** (per-system and fleet-wide) with real computed
  ...
  explanation, plus a **PDF export** of both and a **fleet latency-trend
  column** so a rising system doesn't need to be spotted by eye
- **Webhook notifications** (opt-in, off by default) — a background check
  posts to a URL you configure when a metric newly crosses watch/critical or
  recovers, compatible with Slack/Teams/PagerDuty/any automation tool that
  accepts a webhook
- **Scheduled reports**, generated and archived automatically on a
  daily/weekly cadence, browsable from the Config tab
- **Persistent findings history and acknowledgment** — survives restarts, so
  a known issue can be marked seen without hiding it from the dashboard, and
  "what resolved recently" is answered without re-deriving it from raw
  metrics
- **Maintenance windows** to mute notifications for a planned change,
  without touching what the dashboard or reports actually show
...
| **Check-and-notify, with one explicit exception** | The update checker (`internal/updates`) makes outbound calls to check for newer releases of bundled components and vendor references — for those, it never downloads, installs, or modifies anything. Plumb's own version is the one thing that can be acted on: clicking **Update now** downloads and applies a new release (see the next row). The whole capability — checking and updating both — is fully disabled by `PLUMB_CHECK_FOR_UPDATES=false`, for an air-gapped deployment |
| **Self-update has no code-signing verification** | `internal/selfupdate` downloads over HTTPS from `api.github.com`/GitHub's release CDN and confirms the archive extracts to a non-empty executable — it does not verify a cryptographic signature, since there's no signing infrastructure for this project to check against. The trust boundary is the same one `run.sh`/`run.ps1` already rely on: HTTPS transport and this GitHub repo not being compromised. It never touches the currently-running install directory — a bad update is reversible by stopping the new process and starting the old one, which is left exactly as it was |
...
| **Webhook notifications are opt-in and off by default** | Disabled until you enter a URL and turn it on in the Config tab. Once enabled, a background check (`internal/notify`) posts array/metric names, severity, and finding text — not raw historical data — to the URL you configure, on a new/escalated/resolved finding. Nothing is sent anywhere until you provide a destination |
...
| **"Update now" never reconnects** | The new version failed to start (corrupted download, disk full, or a genuine bug) | The old install directory is untouched — go back to it and run `./start.sh`/`start.bat` there while you check `data/logs/plumb.log` inside the new sibling directory (`plumb-<version>-update/`) for what went wrong |
